...
- **今日任务**：当天新建任务清单，支持添加（可指定每番茄时长 25/30/45/60 分钟与计划番茄数）、行内编辑 / 删除 / 设为当前任务；跨天自动清零，昨天的待办不带到今天
- **按任务计时**：工作段的番茄时长优先使用当前激活任务的指定时长（未指定则用全局设置）
...
简要说明（以 LICENSE 全文为准）：允许任意使用、复制、修改、合并、发布、分发、再许可和售卖；使用者需保留版权声明与本许可声明；本项目按"现状"提供，不附带任何明示或默示的担保。
